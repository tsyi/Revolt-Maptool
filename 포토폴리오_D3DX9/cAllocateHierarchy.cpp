...
STDMETHODIMP cAllocateHierarchy::CreateFrame( THIS_ LPCSTR Name, LPD3DXFRAME *ppNewFrame )
{
	...
	if(Name)
	{
		...
		strcpy_s(pBone->Name, nLen * sizeof(char), Name);
	}
	...
	LPD3DXMESHCONTAINER *ppNewMeshContainer)
{
	...
	for (DWORD i = 0; i < NumMaterials; ++i)
	{
		...
		LPDIRECT3DTEXTURE9 pTex = MgrTexture->GetTexture(sFullPath);
	...
	if (pSkinInfo)
	{
		...
		pBoneMesh->dwNumPaletteEntries = (((m_dwDefaultPaletteSize) < (pSkinInfo->GetNumBones())) ? (m_dwDefaultPaletteSize) : (pSkinInfo->GetNumBones()));
	...
	if(pSkinInfo)*ppNewMeshContainer = pBoneMesh;
	else *ppNewMeshContainer;
