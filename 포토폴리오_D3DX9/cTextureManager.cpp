...
LPDIRECT3DTEXTURE9 cTextureManager::GetTexture(char * szFullPath)
{
	...
	{
		D3DXCreateTextureFromFileA(MgrD3DDevice,szFullPath,&m_mapTexture[szFullPath]
		);
	}
...
{
	if (m_mapTexture.find(szFullPath) == m_mapTexture.end() ||
		m_mapImageInfo.find(szFullPath) == m_mapImageInfo.end()) // 없으면... 코드셍행
	{
		SAFE_RELEASE(m_mapTexture[szFullPath]);
		D3DXCreateTextureFromFileExA(MgrD3DDevice, szFullPath,
			D3DX_DEFAULT_NONPOW2, D3DX_DEFAULT_NONPOW2, D3DX_DEFAULT,
			0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_FILTER_NONE, D3DX_DEFAULT, 0,
			&m_mapImageInfo[szFullPath], NULL, &m_mapTexture[szFullPath]);
	}
